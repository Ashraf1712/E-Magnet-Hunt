...
public class BoardDataDrawer : Editor
{
    ...
    private ReorderableList _questionList;

    private void OnEnable()
    {
        initializeReorderList(ref _dataList, "SearchWords", "Searching Answer");
        initializeQuestionReorderList(ref _questionList, "SearchWords", "Search Question");
    }
    ...
    private void initializeQuestionReorderList(ref ReorderableList list, string propertyName, string listLabel)
    {
        list = new ReorderableList(serializedObject, serializedObject.FindProperty(propertyName), true,
            true, true, true);
        ...
        list.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
        {
            ...
            rect.y += 2;
            EditorGUI.PropertyField(new Rect(rect.x, rect.y, EditorGUIUtility.labelWidth, EditorGUIUtility.singleLineHeight),
                element.FindPropertyRelative("questions"), GUIContent.none);
        };
    }
