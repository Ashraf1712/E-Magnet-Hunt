...
public class WordsGrid : MonoBehaviour
{
    ...
    { 
        ...
        {
            if(rowNumber + 1 > currentGameData.puzzleBoardData.Rows)
            {
                columnNumber++;
                rowNumber = 0;
            }
    ...
    private Vector2 GetFirstSquarePosition()
    {
        ...
        var midWidthPosition = (((currentGameData.puzzleBoardData.Columns - 1) * squareSize.x) / 2) * 0.01f + 5f;
        var midHeightPosition = (((currentGameData.puzzleBoardData.Rows - 1) * squareSize.y) / 2) * 0.01f;
    ...
    private void SpawnGridSquares()
    {
        if (currentGameData != null)
        {
            ...
            foreach(var square in currentGameData.puzzleBoardData.Board)
            {
    ...
    private Vector3 getSquareScale(Vector3 defaultScale)
    {
        ...
        while (ShouldScaleDown(finalScale))
        {
            ...
            if(finalScale.x <= 0.0f || finalScale.y <= 0.0f) 
            {
                finalScale.x = adjustment;
                finalScale.y = adjustment;
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        var midWidthPosition = ((currentGameData.puzzleBoardData.Columns * squareSize.x) / 2) * 0.01f;
        var midHeightPosition = ((currentGameData.puzzleBoardData.Rows * squareSize.y) / 2) * 0.01f;
